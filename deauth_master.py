...
def scan(wlan_interface):
    ...
    try:
        ...
        while True:
            ...
            network_id = []
            for iindex, item in enumerate(active_wireless_networks):
...
def main():
    while True:
        ...
        bssids = scan(wlan_interface)

        print("\nPress CTRL-C to stop the attack")

        start_deauth(wlan_interface, bssids)     # Start attack

        while True:
            response = input("Do you want to make another Deauthenfication? (y/n)").strip().lower()
            if response == 'y':
                print("Let's start another attack!")
                break
            elif response == 'n':
